...
SAFETY_ENV_IDS = [
    f"Safety{agent}{task}{level}-v0"
    for agent in ["Point", "Car", "Ant"]
...
def train(args, use_shaping):
    ...
    assert isinstance(envs.single_action_space, gym.spaces.Box), "PPO Safety expects continuous Box actions."
    assert isinstance(envs.single_observation_space, gym.spaces.Box), "PPO Safety expects Box observations."
    ...
    recent_cost = deque(maxlen=100)
    # Per-env cumulative cost tracker for the current episode
    episode_cost = np.zeros(args.num_envs, dtype=np.float64)
    global_step = 0
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(args.num_steps):
            ...
            # Accumulate per-step cost from safety_gymnasium
            for env_idx in range(args.num_envs):
                step_cost = 0.0
                if "cost" in infos:
                    step_cost = float(infos["cost"][env_idx])
                episode_cost[env_idx] += step_cost
            ...
            if "final_info" in infos:
                for env_idx, info in enumerate(infos["final_info"]):
                    ...
                    ep_cost = float(episode_cost[env_idx])
                    recent_cost.append(ep_cost)
                    episode_cost[env_idx] = 0.0
                    ...
                    writer.add_scalar(
                        "charts/mean_cost",
                        float(np.mean(recent_cost)) if recent_cost else 0.0,
                        global_step,
                    )
        ...
        mean_cost = float(np.mean(recent_cost)) if recent_cost else 0.0
